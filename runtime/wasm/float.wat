(module
   ...
   (func (export "caml_float_of_string") (param (ref eq)) (result (ref eq))
      (local $s (ref $string)) (local $len i32) (local $i i32)
      (local $negative i32) (local $c i32)
      (local $f f64)
      ;; ZZZ
      ;; count underscores; if underscores, reallocate the string
      ;; check for nan or inf(inity)
      ...
      (block $error
         ...
         (if (i32.lt_u (i32.add (local.get $i) (i32.const 2)) (local.get $len))
            (then
               (if (i32.eq (array.get $string (local.get $s) (local.get $i))
                           (i32.const 48)) ;; '0'
                  (then
                     (if (i32.eq (i32.and

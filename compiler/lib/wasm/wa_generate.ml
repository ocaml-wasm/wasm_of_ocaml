...
module Generate (Target : Wa_target_sig.S) = struct
  ...
  let find_debug_loc ctx ?force loc =
    match Parse_bytecode.Debug.find_loc ctx.debug ?force loc with
    | None | Some { Parse_info.src = None | Some ""; _ } -> None
    | loc -> loc
  ...
  and translate_instr ctx context (i, loc) =
    with_location
      (find_debug_loc ctx ~force:Not_alloc loc)
      (match i with
